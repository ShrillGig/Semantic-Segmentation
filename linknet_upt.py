...
def encoder_block(x, input_filters: int):
     ...
     shortcut_1 = Conv2D(input_filters, kernel_size=(1, 1), strides=2, use_bias=False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     shortcut_1 = BatchNormalization()(shortcut_1)
    
     # First convolution layer with dimensionality reduction (strides=2)
     x = Conv2D(input_filters, kernel_size=(3, 3), strides=2, use_bias=False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     x = BatchNormalization()(x)
     x = ReLU()(x)

     # Second convolution layer
     x = Conv2D(input_filters, kernel_size=(3, 3), use_bias=False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     x = BatchNormalization()(x)
     x = ReLU()(x)
...
def decoder_block(x, skip_connection, input_filters, output_filters: int):
     ...
     x = Conv2D(input_filters // 4, kernel_size=(1, 1), use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     x = BatchNormalization()(x)
     x = ReLU()(x)
     
     # 3x3 transposed convolution → increase in spatial size
     x = Conv2DTranspose(input_filters // 4, kernel_size=(3,3), strides=2, use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     x = BatchNormalization()(x)
     x = ReLU()(x)
     ...
     x = Conv2D(output_filters, kernel_size=(1, 1), use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(x)
     x = BatchNormalization()(x)
...
def linknet_model(n_classes=4, IMG_HEIGHT=128, IMG_WIDTH=128, IMG_CHANNELS=1): #Don't forget to change n_classe if necessary
    ...
    #Initial block
    initial = Conv2D(64, kernel_size=(7,7), strides=2, use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(inputs)
    ...
    #Final block
    out = Conv2DTranspose(32, kernel_size=(3,3), strides=2, use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(d1)
    out = BatchNormalization()(out)
    out = ReLU()(out)
    
    out = Conv2D(32, kernel_size=(3,3), use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(out)
    out = BatchNormalization()(out)
    out = ReLU()(out)
    
    out = Conv2DTranspose(n_classes, kernel_size=(2,2), strides=2, use_bias = False, padding='same', kernel_initializer='he_normal', kernel_regularizer=regl)(out)
    out = BatchNormalization()(out)
    outputs = Softmax()(out)
